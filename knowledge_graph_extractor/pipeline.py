...
# -------------------------------
# Internal imports from package
# -------------------------------

from knowledge_graph_extractor.llm import llm_call, chunking_llm_call, salvage_json
from knowledge_graph_extractor.prompts import (
    extraction_sys_edge, 
    extraction_usr_edge, 
    extraction_sys_node, 
    extraction_usr_node, 
...
from knowledge_graph_extractor.graph_ops import (
    add_metadata_to_nodes, 
    add_metadata_to_edge, 
    node_to_db, 
    edge_to_db, 
    episodic_generator, 
    advanced_deduplication_controller
)
...
async def processing_node_edge(text: list, db_name: str):
    ...
    """
    Orchestrates the processing of a list of articles.
    Each article is chunked and processed to extract nodes and edges.
    Args:
        text (list): List of articles in dictionary form, e.g., [{"article_0": "..."}]
        db_name (str): Database name where nodes and edges will be stored.
    """
    ...
    for i in range(0, len(text)):
        ...
        print("Waiting 20 second to respect rate limits...\n")
        await asyncio.sleep(20)

# ---------------------------------------
# Helper function for processing one chunk
# ---------------------------------------

async def each_ep_generator_node_edge(data, db_name, ep_name):
     
    """
    Processes a single chunk of text: generates nodes, edges, episodic data,
    stores them in the database, and performs deduplication.
    Args:
        data (str): Text chunk to process
        db_name (str): Database name
        ep_name (str): Unique name for this chunk / episode
    """
     
	# ----------------------
    # Step 1: Generate Nodes
    # ----------------------
    # Use the LLM to extract key entities (nodes) from the text chunk
    ...
    # ----------------------
    # Use the LLM to extract key relationships (edges) from the text chunk
    ...
    # ----------------------
    # Episodic nodes summarize context and relationships for this chunk
